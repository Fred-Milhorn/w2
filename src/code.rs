...
fn fixup_invalid(body: Instructions) -> Instructions {
	...
	match instruction {
		...
		instructions.push(Instruction::Cmp(src1,
						   Operand::Reg(Register::R11)));
	    },
	    Instruction::Cmp(Operand::Stack(src1_offset), Operand::Stack(src2_offset)) => {
		instructions.push(Instruction::Mov(Operand::Stack(src1_offset),
						   Operand::Reg(Register::R10)));
		instructions.push(Instruction::Cmp(Operand::Reg(Register::R10),
						   Operand::Stack(src2_offset)));
	    },
	    Instruction::Mov(Operand::Stack(src_offset), Operand::Stack(dst_offset)) => {
		instructions.push(Instruction::Mov(Operand::Stack(src_offset),
						   Operand::Reg(Register::R10)));
		instructions.push(Instruction::Mov(Operand::Reg(Register::R10),
		...
		instructions.push(Instruction::Mov(Operand::Stack(dst_offset),
						   Operand::Reg(Register::R11)));
		instructions.push(Instruction::Binary(BinaryOperator::Mult,
						      src,
						      Operand::Reg(Register::R11)));
		instructions.push(Instruction::Mov(Operand::Reg(Register::R11),
						   Operand::Stack(dst_offset)));
	    },
...
fn emit_function(code: &mut String, name: &str, instructions: &Instructions) -> Result<()> {
	...
	match instruction {
		...
		writeln!(code, "\tjmp\tL{}", label)?
	    },
	    Instruction::Label(label) => {
		writeln!(code, "L{}:", label)?
	    },
	    Instruction::JmpCC(cc, label) => {
		writeln!(code, "\tj{}\tL{}", cc.name(), label)?
	    },
	    ...
	    Instruction::AllocateStack(number) => {
		writeln!(code, "\tsubq\t${number}, %rsp")?
	    },
	    Instruction::Ret => {
		writeln!(code, "\tmovq\t%rbp, %rsp\n\tpopq\t%rbp\n\tret")?
	    },
	    Instruction::Cdq => {
		writeln!(code, "\tcdq")?;
	    },
